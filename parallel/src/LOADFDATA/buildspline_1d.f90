subroutine buildspline_1d (integral, numz, itype, in1, in2, zmax, interaction, xintegral_2c)
  ...
  if(((interaction .eq. 12) .or. (interaction .eq. 4)) .and. (itype .ne. ind2c(4,0))) then
    d(numz_used+1) = -3.0*h*(xintegral_2c(integral,numz_used+1) - xintegral_2c(integral,numz_used))/zmax - w*d(numz_used)
  else
    d(numz_used+1) = -w*d(numz_used)
  end if

  z(numz_used+1) = d(numz_used+1)/b(numz_used+1)
  do iorder=numz_used,1,-1
