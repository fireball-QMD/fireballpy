...
subroutine set_coords(natoms, z, xyz)
  use M_system
  use M_fdata, only : symbolA, nspecies, nzx 
  implicit none
  integer, intent(in) :: natoms
  integer, dimension(natoms), intent(in) :: z
...
subroutine info_forces(out_forces)
  use M_system
  implicit none
  real*8, dimension(natoms, 3), intent(out) :: out_forces
  integer :: iatom
...
subroutine info_charges(out_charges)
  use M_system, only : natoms, Qin, imass
  use M_fdata, only : nssh
  implicit none
  real*8, dimension(natoms), intent(out) :: out_charges
  integer :: iatom, issh
  real*8 :: out_qsum
  do iatom = 1, natoms
    out_qsum = 0.0d0
