...
!JOM
        real(kind=dp), intent (in), dimension (nssh) :: rcutoff_gs
        real(kind=dp), dimension (:), allocatable :: rcutoff_psirc
...
!JOM
        ...
        real(kind=dp), dimension (:, :), allocatable :: psi1_new
...
! Construct r at each mesh point.
        ...
        if (abs(r(mesh) - rc_max) .gt. 1.0e-6_dp)                              &
     &   stop ' bad agreement between r(mesh) and rc '

!JOM
        do issh = 1, nssh
         mesh_psirc(issh) = int((rcutoff(issh) + 1.0e-4_dp)/dr) + 1
...
!JOM : mix GS and DMOL
   ...
   &                              + (1.0_dp-cmix(issh))*psi(issh,ipoint)
         end do
...
!         open (unit = 14, file = filename_ewf(issh), status = 'unknown')
         if(sav(issh)) open (unit = 14, file = trim(outpath)//trim(filename_wf(issh)), status = 'unknown')
         if(sav(issh)) write(14,101) filename_wf(issh)
         if(sav(issh)) write(14,102) nznuc, atomname
         if(sav(issh)) write(14,103) mesh_new
         zero = 0.0_dp
!JOM
	 rc_dp = max(rcutoff_psirc(issh),rcutoff_gs(issh))
!
! charges xocc for the calculation of NA_potential
!JOM
!         if(sav(issh)) write(14,104) rcutoff_psirc(issh), rc_max, zero
         if(sav(issh)) write(14,104) rc_dp, rc_max, xocc0(issh)
...
!dani          write (18,401) r(ipoint), psi(issh,ipoint)/(abohr1_5*r(ipoint))
...
!JOM
!         if(sav(issh)) write(14,104) rcutoff_psirc(issh), rc_max, zero
         if(sav(issh)) write(14,104) rc_dp, rc_max, 0.0_dp
         if(sav(issh)) write(14,105) lam(issh)
...
! Find (approximately) the value at r=0
         ...
         yy(2) = psi1_new(issh,2)/(abohr1_5*r(2))
         yy(3) = psi1_new(issh,3)/(abohr1_5*r(3))
         ...
         xx(4:mesh_new-1) = r(4:mesh_new-1)*abohr
         yy(4:mesh_new-1) = psi1_new(issh,4:mesh_new-1)/(abohr1_5*r(4:mesh_new-1))
