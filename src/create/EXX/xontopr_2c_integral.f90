...
! Program Declaration
! ===========================================================================
        subroutine xontopr_2c_integral (fraction, nalpha, lalpha, malpha,   &
...
! all of the states at a given site, for the ontop case this is a sum over
...
! We are integrating the r' integral in cylindrical coordinates here so the
! phi integration only goes from 0 - pi, rather than 0 - 2*pi; therefore, we 
! divide by an extra factor of two here (compared to the xontop-left case).
          cg1 = clebsch_gordon (lalpha, 0, lqn, 0, l2, 0)
          cg2 = clebsch_gordon (lalpha, malpha, lqn, mqn, l2, m2)
          phifactor = cg1*cg2*clm(l1,m1)*clm(lalpha,malpha)*clm(lqn,mqn)     &
     &                   *delk(m1,mqn+malpha)*sqrt(2.0_dp*real(lalpha)+1.0_dp) &
     &                   /(sqrt((2.0_dp*real(lqn) + 1.0_dp)                    &
     &                         *(2.0_dp*real(l2) + 1.0_dp))*4.0_dp)
...
! pi, delta, or phi.
             if ((r1 .lt. rcutoff1) .and. (r2 .lt. rcutoff2)) then
              psi1 = wf_atoms(itype1)%get_psi( n1, r1)
              psi2 = wf_atoms(itype2)%get_psi( nalpha, r2)
...
! Obtain the r dependent answer by interpolation.
              sumrp = rprimeofr (r1, lqn, n2, rcutoff2)
