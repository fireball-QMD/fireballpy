...
! arrays which determine non-zero matrix elements
        ...
        character(len=2) atom1
        character(len=2) atom2
        character(len=70) what1
        character(len=70) what2
...
! Set up the header strings:
        ...
        if (interaction .eq. 0) then
         ...
         write (iounit,*) &
     &             ' (Non)-neutral atom potential (ontop) two-center'
        else if (interaction .eq. 3) then
         write (iounit,*) &
     &             ' (Non)-neutral atom potential (atom) two-center'
         ...
         write (iounit,*) &
     &          ' Extended Hubbard: nu12 = int[n1*n2*nu(n1+n2)]'
        else if (interaction .eq. 13) then
         write (iounit,*)  &
...
! orbitals, i.e. this is dna.
     ...
     &       .or. interaction .eq. 13 .or. interaction .eq. 14 ) then
