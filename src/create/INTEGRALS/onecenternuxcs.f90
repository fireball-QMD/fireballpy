...
! correlation interactions of the extended Hubbard model - spin polarizaiton.
!
...
! Program Declaration
! ===========================================================================
        subroutine onecenternuxcs (nspec, nspec_max, nsh_max, wfmax_points,  &
...
! Set up the header for the output file.
        write (36,100)
        write (36,*) ' All one center matrix elements (spin) '
        write (36,*) ' created by: '
...
! Convert to eV
          dnuxc_s = hartree*dnuxc_s*abohr**3
 
          do issh = 1, nssh
           do jssh = 1, nssh
            answer(issh,jssh) = answer(issh,jssh)                            &
     &       + dnuxc_s*factor*rho**2*(psiofr(in1,issh,rho)**2)             &
