...
! Program Description
! ===========================================================================
!       This subroutine takes a given root and suffix with two index numbers.
...
! Find true length of root and suffix
        ...
        if (lr + ls + nplace + 1 .gt. len(filename)) then
         ...
         write (6,*) root, suffix, index1, index2, iunit
...
! Write out the index numbers with a '.' between each number
        filename(lr+1:lr+1) = '.'
        write (filename(lr+2:lr+nplace+1),form) index1
        filename(lr+nplace+2:lr+nplace+2) = '.'
        write (filename(lr+nplace+3:lr+2*nplace+2),form) index2
...
! Fill rest with spaces in case there is garbage there now
        do i = lr + 2*nplace + ls + 4, len(filename)
         filename(i:i) = ' '
        end do
