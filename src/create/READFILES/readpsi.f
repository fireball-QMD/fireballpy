...
! Set some things up
        ...
        drr(issh,ispec) = rc/dfloat(mesh - 1)
        ...
        rrc_rho(ispec) = dfloat(npoints_rho(ispec) - 1)*drr_rho(ispec)
...
! interactions for gradients go haywire at the endpoints where rho = 0.0d0
        ...
        do ipoint = 1, npoints_rho(ispec)
         rr_rho(ipoint,ispec) = dfloat(ipoint - 1)*drr_rho(ispec)
        end do
...
! Check normalization
     ...
     1                       psi_spline(1,1,issh,ispec), mesh, rcutoff)
