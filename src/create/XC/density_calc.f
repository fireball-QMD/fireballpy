...
! ======================================================================
     ...
     1                           itype3, r, dr, dens, ddens, dddens)
        implicit none
...
! ***************************************************************************
     ...
     1      .or. iexc .eq. 9 .or. iexc .eq. 10) then
         ...
         do issh = 1, nsshxc(itype)
          densdr =
     1     densdr + xnocc(issh,itype)*psiofr(itype,issh,r+dr)**2
          dens_dr =
     1     dens_dr + xnocc(issh,itype)*psiofr(itype,issh,r-dr)**2
         end do

         if (j1ch .eq. ispec) then
          densdr =
     1     densdr + j2(ix)*dqorb(j1at)*psiofr(j1at,jssh,r+dr)**2
          dens_dr =
     1     dens_dr + j2(ix)*dqorb(j1at)*psiofr(j1at,jssh,r-dr)**2
         end if
...
! At the endpoint do a forward difference. First, we need the point at r+2dr.
          ...
          do issh = 1, nsshxc(itype)
           dens2dr =
     1      dens2dr + xnocc(issh,itype)*psiofr(itype,issh,r+2.0d0*dr)**2
          end do

          if (j1ch .eq. ispec) then
           dens2dr = dens2dr
     1      + j2(ix)*dqorb(j1at)*psiofr(j1at,jssh,r+2.0d0*dr)**2
          end if
