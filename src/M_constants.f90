module M_constants
  ...
  ! Float precission especification
  ...
  ! Bohr constant
  real(wp), parameter :: abohr = 0.529177210544d0
  real(wp), parameter :: eq2 = 14.39975d0
  real(wp), parameter :: Hartree = 14.39975d0/abohr

  ! Rydberg/eV conversion
  real(wp), parameter :: ryd = 13.6d0

  ! Boltzmann's constant
  real(wp), parameter :: kb = 8.617343693082104d-5! 1.380662d-23 J/K * 6.24150974*10^18eV/J
  ...
  !        real(wp), parameter :: fovermp = 0.0095790d0
  real(wp), parameter :: fovermp = 0.009648957597d0

  real(wp), parameter :: kconvert = 11604.49558d0
  real(wp), parameter :: pi = 3.141592653589793238462d0
  real(wp), parameter :: spin = 2.0d0

  ! Gear algorithm constants_fireball
  !         integer, parameter :: gear_order = 5  !  (only use 2-7)
  !         real(wp), dimension (0:gear_order) :: cfac
  ! JOM-info I change this so that I can run verlet (gear_order=2)
  integer, parameter :: gear_order = 2  !  (only use 2-7)
  real(wp), dimension (0:5) :: cfac

  ! Kronecker delta
  real(wp), dimension (3, 3) :: delk

  ! Levi-Civita parameter
  real(wp), dimension (3, 3, 3) :: xlevi
  ...
  ! Note: what we used to call 6th order was real(wp)ly 5th!
