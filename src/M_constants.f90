module M_constants
  ...
  ! Bohr constant
  real(c_double), parameter :: abohr = 0.529177210544d0
  real(c_double), parameter :: eq2 = 14.39975d0
  real(c_double), parameter :: Hartree = 14.39975d0/abohr

  ! Rydberg/eV conversion
  real(c_double), parameter :: ryd = 13.6d0

  ! Boltzmann's constant
  real(c_double), parameter :: kb = 8.617343693082104d-5! 1.380662d-23 J/K * 6.24150974*10^18eV/J
  ...
  !        real(c_double), parameter :: fovermp = 0.0095790d0
  real(c_double), parameter :: fovermp = 0.009648957597d0

  real(c_double), parameter :: kconvert = 11604.49558d0
  real(c_double), parameter :: pi = 3.141592653589793238462d0
  real(c_double), parameter :: spin = 2.0d0

  ! Gear algorithm constants_fireball
  !         integer(c_long), parameter :: gear_order = 5  !  (only use 2-7)
  !         real(c_double), dimension (0:gear_order) :: cfac
  ! JOM-info I change this so that I can run verlet (gear_order=2)
  integer(c_long), parameter :: gear_order = 2  !  (only use 2-7)
  real(c_double), dimension (0:5) :: cfac

  ! Kronecker delta
  real(c_double), dimension (3, 3) :: delk

  ! Levi-Civita parameter
  real(c_double), dimension (3, 3, 3) :: xlevi
