subroutine interpolate_2d (xin, yin, iauxforce, nx, ny, hx, hy, xintegral, Q_L, dQ_Ldx, dQ_Ldy)
  ...
  real(wp), intent (in) :: hy
  real(wp), intent (in), dimension (numXmax, numYmax) :: xintegral
  real(wp), intent (out) :: Q_L    ! the contibutions for a matrix element
  real(wp), intent (out) :: dQ_Ldx ! d/dx Q_L (Computed only if iauxforce = 1)
  real(wp), intent (out) :: dQ_Ldy ! d/dy Q_L (Computed only if iauxforce = 1)
  real(wp), parameter :: tiny = 1.0d-5
  real(wp), parameter :: small= 1.0d-4
  integer imidx, imidy
  integer k
  real(wp) f0p3, f0p6, f1m1, f1m2, f1m3, f1p3, f1p6, f2p1, flm1
  real(wp) gradtest
  real(wp) gradx, grady
  real(wp) px, py
  real(wp), parameter :: xmin = 0
  real(wp), parameter :: ymin = 0
  real(wp) bb0,bb1,bb2,bb3
  real(wp), dimension (-1:2,-1:2) :: fun
  real(wp), dimension (-1:2) :: g, gp
